...
// --- Component for Individual Quantity Input with Icons ---
function QuantityInput({ initialQuantity, rowKey, onQuantityChange }) {
  const minQuantity = 1;
  // Use a local state for immediate UI feedback on button clicks
  const [quantity, setQuantity] = useState(initialQuantity);

  // Sync internal state if initialQuantity changes (e.g., if parent forces a re-render)
  React.useEffect(() => {
    setQuantity(initialQuantity);
  }, [initialQuantity]);

  // Handler for the Up/Down Icon buttons
...
    // Only update local state and notify parent if the input is a valid number >= min
...
  return (
    <Box
      // Main container to define the single bordered box
      sx={{
        display: "flex",
...
        sx={{
          flexGrow: 1,
          // Hiding the standard MUI input field underlines
          "& .MuiInput-underline:before": { display: "none" },
          "& .MuiInput-underline:after": { display: "none" },
          // Hiding the Webkit spin buttons (Chrome/Safari/Edge)
          "& input": {
...
            // Target the actual input element's style within the wrapper
            "& input": {
...
              // Match Line Height to Container Height for reliable vertical centering
              lineHeight: "40px",
            },
          },
        }}
...
        {/* Up Arrow (Increment) */}
        <IconButton
          // Removed size="small" to prevent conflicting internal padding
 ...
 */
export function CartTable({ onDataChange }) {
  ...
  const handleRemoveItem = useCallback(
    (key) => {
      ...
      const removedItem = cartData.find((item) => {
        const itemKey =
          item.id || item.title + (item.color || "") + (item.size || "");
        return itemKey === key;
      });
